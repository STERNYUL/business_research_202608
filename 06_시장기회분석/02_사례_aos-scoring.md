...
## 5. ⑤ Opportunity Matrix — 기준점 A안 (척도 기반 3.0)

**X축: Satisfaction · Y축: Importance · 기준선 3.0 / 3.0 · AOS 기회선 2.5**

```mermaid
quadrantChart
    title AOS Opportunity Matrix (기준점 3.0)
...
| 사분면 | 조건 | 항목 | 전략 행동 |
|---|---|---|---|
| 🔥 **Q1 혁신기회** | I ≥ 3, S < 3 | **12종** — MK-1 · MK-2 · MK-3 · KD-1 · KD-2 · PS-1 · PS-3 · JM-1 · JM-2 · JM-3 · NG-3 · BS-1 | JTBD 인터뷰 대상, MVP 실험 우선 |
| 💎 **Q2 개선기회** | I ≥ 3, S ≥ 3 | **6종** — KD-3 · NG-2 · BS-2 · BS-3 · PS-2 · NG-1 | 지속적 개선 / 현 수준 유지 |
| ⚫ **Q3 유지관리** | I < 3, S < 3 | **0종** | — |
| ⚠️ **Q4 과잉투자** | I < 3, S ≥ 3 | **0종** | — |

> ⚠️ **Q3·Q4가 비어 있는 것은 우연이 아닙니다.** Pain을 **여정지도의 `이탈 지점` 열에서만** 뽑았기 때문입니다. 사람이 실제로 멈추는 자리는 정의상 중요도가 낮을 수 없어, 추출 방식이 **Importance의 하한(3점)을 만들어냈습니다.** 사분면의 아래쪽 절반이 빈 것은 시장의 성질이 아니라 **우리 추출 규칙의 그림자**입니다. 이 편향을 줄이려면 이탈 지점 외에 `생각/감정` 열이나 인터뷰의 사소한 불만까지 넣어야 합니다.

## 6. 참고 — 기준점 B안 (데이터 분포)으로 다시 그리면

18종의 분포로 계산한 값입니다.

| 항목 | 계산 | 값 |
|---|---|---:|
| Importance 평균 (μ₁) | 78 / 18 | **4.33** |
| Satisfaction 평균 (μ₂) | 38 / 18 | **2.11** |
| AOS 평균 | 44.6 / 18 | **2.48** |
| AOS 표준편차 (σ) | — | **0.83** |
| **AOS 기준선** | 평균 + 0.5σ | **2.89** |

| 사분면 | 조건 | 항목 |
|---|---|---|
| 🔥 **Q1** | I ≥ 4.33, S < 2.11 | **4종** — MK-1 · MK-2 · PS-1 · JM-1 |
| 💎 **Q2** | I ≥ 4.33, S ≥ 2.11 | 4종 — KD-3 · NG-2 · BS-2 · PS-2 |
| ⚫ **Q3** | I < 4.33, S < 2.11 | 8종 — KD-1 · KD-2 · PS-3 · JM-2 · JM-3 · MK-3 · NG-3 · BS-1 |
| ⚠️ **Q4** | I < 4.33, S ≥ 2.11 | 2종 — BS-3 · NG-1 |

**A안과 B안의 Q1이 12종 → 4종으로 줄었습니다.** 기준선을 옮기면 결론이 바뀐다는 것을 그대로 보여줍니다. 다만 **AOS 기준선(2.89)을 통과한 항목은 A안의 2.5 기준과 동일한 8종**입니다 — 사분면은 크게 흔들려도 **AOS 순위는 흔들리지 않았습니다.**

> ⚠️ **B안은 참고 계산일 뿐입니다.** 여기 쓰인 분포는 시장에서 수집된 것이 아니라 **분석자 한 명이 매긴 가설 점수**입니다. 평균·표준편차를 내면 시장의 분포가 아니라 채점 습관을 기준선으로 삼게 됩니다. 실제 인터뷰 데이터가 쌓이기 전까지는 **A안을 주 기준으로 사용합니다.**
...
남기훈의 Build vs Buy 저항은 **크지만 기회가 아닙니다.** 사내 DB로 목표를 이미 달성하고 있어(S=4) 불만이 없기 때문입니다. **이탈 지점과 기회는 다른 것**이고, 여기서 나오는 처방도 다릅니다 — 정면으로 설득하는 대신 [백서현의 보고서 각주로 우회](../05_페르소나/03_사례_고객여정지도.md)하는 전략이 숫자로도 지지됩니다.
...
Q1이면서 AOS 2.5 이상인 **8종**이 [07 JTBD 인터뷰](../README.md)의 1차 대상입니다.

| 우선 | ID | 인터뷰에서 확인할 것 |
|:---:|---|---|
| 1 | **MK-1** | "근거가 어느 수준이면 쓰겠는가" — 등급인지, 사례 목록인지, 신뢰구간인지 |
| 2 | **JM-2** | 소급 검증에서 **몇 년치**를 요구하는가 |
| 3 | **JM-3** | 파일 배치면 **실제로 심사를 통과하는가** |
| 4 | **KD-2** | 반려 사유의 실제 목록 |
| 5 | **PS-3** | "미산출 선언"을 감시 시스템이 받아들일 수 있는가 |
| 6 | **PS-1** | 괴리 설명의 허용 지연 시간 |
| 7 | **JM-1** | 외부 값을 내부 평가와 **병행할 의사**가 있는가 |
| 8 | **MK-2** | 산출 불가를 받으면 **이탈하는가, 수용하는가** |
