...
> **Institutional Valuation Infrastructure** — 상업용 부동산 · 디지털 자산 · 조각투자/토큰증권(STO) 기초자산의 시장가치를 지속 산출해 API·파일로 금융기관 시스템에 공급하는 **평가 데이터 인프라**

---

## 0. 문서 메타데이터

**이 문서는 단독으로 완결됩니다.** 산업 구조 분석부터 기능 명세·출처까지, 판단에 필요한 근거를 **다른 문서를 열지 않고** 읽을 수 있도록 본문에 직접 담았습니다. 외부 1차·2차 출처 URL만 §18에 남겼습니다.

| 항목 | 내용 |
|---|---|
| **문서 성격** | 두 산출물의 **합본** — GPT 계열 산출물의 구조·표준 프레임워크 + Claude 계열 산출물의 정량 근거·경쟁 분석. 여기에 저장소 01~07 챕터의 근거를 전량 내재화 |
| **병합 근거** | §0.1 |
| 버전 | **v1.0 (Merged · Rooted)** — 외부 참조 제거, 근거 내재화 |
| 작성일 | 2026-08-14 |
| 논리 순서 | `Pain → Goal → Job → Outcome → Alternative → Value → Feature → MVP` |
| Pain ID 체계 | **저장소 정본 유지** — `KD·PS·JM·MK·NG·BS·IS·CH` |
| Feature 번호 | **F1~F25** |
| 페르소나 | **최종 로스터 12종** *(핵심 3 · 확장 4 · 극단 1 · 비활성 4)* |
...
| 강도 | 뜻 | 이 문서에서의 범위 |
|:---:|---|---|
| ✅ **검증** | 공개 1차·2차 출처 *(경쟁사 자료·제도·통계)* | 경쟁 11사 실적·가치선언 · 전자증권법 시행일 · 감정평가 시장 규모 · 리츠·가상자산 통계 |
| 🔵 **도출** | 저장소 내 계산 *(입력이 가설 점수)* | AOS·DOS · 클러스터 · 사분면 · 밸류체인 · KSF · 분할 점검 |
| 🟡 **가설** | **AI 생성 가상 응답지** — 실제 고객 발언 아님 | 인터뷰 인용문 전부 · Outcome 정량값 · WTP · 전환 판정 · 수용 임계값 |

> 🔴 **"경쟁사가 이걸 안 한다"는 ✅이고, "고객이 이걸 원한다"는 🟡입니다.**
> **차별성은 증명됐고 필요성은 미확인**입니다 — 이 비대칭이 이 문서에서 가장 정직해야 하는 지점입니다.

### 0.1 왜 합본인가 — 두 산출물의 성질이 달랐습니다

같은 과업을 두 모델이 수행했고, **차이의 최대 변수는 모델이 아니라 입력 범위**였습니다.

| | **GPT 계열 산출물** | **Claude 계열 산출물** |
|---|---|---|
| 참조 페르소나 | 5종 | 7종 |
| 대표 Pain | 15개 | 21개 *(AOS 대상 18개)* |
| TAM-SAM-SOM | *"자료에서 확인되지 않음"* → 산정 안 함 | 105.6억 · 23.75억 실수치 |
| AOS·DOS | 언급만 | 전수 계산 |
| 경쟁사 | 유형 7종 *(실명·수치 없음)* | **11사 실명 + 출처 링크 72개** |
| 구조 | **선형 22섹션 — 읽는 문서** | 중첩 9섹션 — 찾아보는 문서 |

**합본이 이식한 것은 셋입니다** — ① **Value Proposition Canvas**(§9, Claude 산출물의 명백한 누락) ② **검증 가설 번호 체계 H1~H12**(§13, 번호가 있어야 *"H5는 확인됐고 H3은 아직"* 이라고 말할 수 있음) ③ **영문 개념 라벨 5종**(§8, 대외 커뮤니케이션 자산). 반대로 정량 근거·경쟁 분석·반증 기록은 Claude 쪽을 정본으로 삼았습니다.

> ⚠️ **다만 GPT 쪽의 미덕 하나는 이 문서가 갖지 못합니다** — *"확인되지 않은 숫자를 만들지 않는다"*. 이 문서는 숫자를 채웠고 **그 상당 부분이 가상 인터뷰(🟡)에서 나왔습니다.** 경고를 달았더라도 **정교함이 신뢰도로 오인될 위험**은 남습니다.
...
## 1.4 산업 구조 — 포터의 기준으로는 매력적이지 않습니다 ✅🔵

**이 선언이 필요한 이유가 산업 구조에 있습니다.**
...
| ⑤ 기존 경쟁 강도 | ★★★☆☆ | 직접 경쟁자는 적으나 **인접 유형이 아홉 갈래** *(§6.2)* |

**다섯 중 셋이 최고 강도입니다.** 수익성은 산업 구조가 아니라 포지셔닝에서 나와야 합니다.

**축별로 다시 채점하면 갈립니다.** 포터의 다섯 힘은 *하나의 산업*을 전제하는데 우리 제품 축은 셋입니다 — 하나의 별점으로 요약하면 **STO 축의 예외가 지워집니다.**
...
> 🔴 **STO 축이 구조적으로 가장 유리합니다** — 최고 강도가 하나(구매자)뿐입니다. 부동산 축은 셋, 디지털 축은 둘입니다. 이유는 §6.5.
>
> ⚠️ **다만 이것이 "STO부터 가라"를 뜻하지 않습니다.** 구조가 유리한 것과 지금 매출이 나는 것은 다릅니다 — **STO 축은 SAM이 아직 산정되지 않았고** 요건이 확정되지 않아 제품 사양도 미정입니다. 지금 할 일은 판매가 아니라 **요건 정의 참여**입니다.
...
| **3** | 3년 내 **인증 칸 조달** *(QRM·보험·하이브리드·오차 이력)* | 내부 참고용 툴로 축소, ARPU 하락 |
| **4** | **평가 엔진이 자산군을 넘어 재사용** | 자산군별 별도 제품 = 단일 버티컬 회사 |

| 사업 성격 | 성립 |
|---|:---:|
| **현금흐름 사업** *(소수 인원, 제품 주도 도입)* | ⭕ |
| **VC 스케일** *(수백억 밸류)* | ✕ **국내 단독으로는 불가** |

> **최악 시나리오는 3·4가 동시에 깨지는 경우입니다** — 국내 CRE 데이터 회사 하나, ARR 10억 내외. **사업은 되지만 벤처는 아닙니다.** VC 스케일이 되려면 셋 중 하나가 필요합니다 — **자산군 확장 · 해외 · 인접 수익원**(하이브리드 평가 수수료, 감정평가법인 리셀). **셋 다 현재는 가설입니다.**

## 1.6 현상 유지를 깨는 힘은 우리 세일즈가 아니라 외부 트리거입니다

*"엑셀로 버티기"* 는 설득으로 깨지지 않습니다. 고객 여정 S1(자각) 트리거를 다시 보면 **상위 Pain 대부분이 규제 또는 사건 트리거를 갖고 있습니다.**

| 항목 | 트리거 | 성격 |
|---|---|---|
| JM-1 · JM-2 · JM-3 | **감독당국 지적·검사** | 규제 |
| PS-1 · PS-3 | **이상거래·상장 사고** | 사건 |
| CH-1 · CH-2 | **감사 품질관리 심사** | 규제 |
| *(서태영 · STO)* | **투자계약증권 판단 · 전자증권법 시행** | 규제 — **최상위** |
| KD-2 | 내부 보안 절차 | 절차 |
| MK-1 · MK-2 | 시연 실패 | **우리가 만드는 것** |

> 💡 **트리거가 없는 세그먼트는 제외했습니다** — 최도윤(기업 부동산 담당) · 일반 부동산 보유자 · **남기훈**. 남기훈 본인의 답이 *"저흰 **감독 검사 대상이 아닙니다.** 투자심의가 우리 감사예요"* 였습니다. 🔴 **조건 1을 세워놓고 남기훈에게 적용하지 않은 것이 이 프로젝트의 누락이었고, 세그먼트 재판정으로 직접 영업 대상에서 제외했습니다.**

---

# 2. 페르소나 · CJM 기반 핵심 문제 (Pain / Needs)

**Pain은 고객 여정지도의 `이탈 지점` 열에서만 추출했습니다** — 사람이 실제로 멈추는 자리가 곧 Pain이라는 규칙입니다.
...
**비활성 장벽이 여섯 종류입니다** — 구조(임성호) · 인식(유가람) · 이해상충 · 가격 · 대체재 우위(최도윤) · **고용 방어(하승우)**. 장벽의 종류가 다르면 해제 수단도 다릅니다.
...
## 2.2 페르소나별 Pain → Needs — 전수 24종 (AOS 대상 21종)

**분할 4종은 두 행으로 표기해 실제 행 수는 25입니다**(§2.5). IS 계열 3종은 AOS를 계산하지 않습니다.
...
| **KD-2** | 보안·준법 심사에서 **문서 미비로 반려.** *"반려된 뒤로 아예 시도를 안 한다"* | 챙기지 않아도 심사 자료가 준비돼 통과되는 상태 | S5 | 4 | 1 | **3.2** |
...
| **여정 특징** | **앞이 짧고 뒤가 깁니다.** 김도현과 정반대 — 계약까지 2~4개월인데 **S6·S7(연동·장애)에서 죽습니다** |
...
| **PS-2** | 상시 결합이라 **공급자 장애가 곧 우리 장애** | 장애가 전파되지 않거나 우리가 먼저 아는 상태 | S7 | 5 | 4 | 1.0 ⚠️ |

> ⚠️ **PS-2는 AOS 최하위인데 KSF 필수 요건(K5 운영 신뢰성)입니다** — 거래소가 지금 겪지 않는 문제라 점수가 낮을 뿐, **우리가 들어가면서 새로 만드는 리스크**입니다. §10.5의 유형 C 참조.
...
| ID | Pain | Needs | CJM | I | S | **AOS** 🔵 |
|---|---|---|:---:|:---:|:---:|---:|
| **JM-2** | *"2년 전 그 시점에 당신 모델은 뭐라 했나"* 를 확인할 방법이 없다. **소명에 두 달** | 과거 임의 시점의 값과 방법론을 재현하는 상태 | S3 | 5 | 1 | **4.0** |
| **JM-1** | 자체 감정평가가 담보가를 높게 잡는다는 지적. 반증할 독립 데이터 없음 | 외부 독립 값으로 보완·반박할 수 있는 상태 | S1 | 4 | 2 | 2.4 |
| **JM-3** | 위탁·보안 이중 심사. **API 전용이면 시작도 못 한다** | 회선을 열지 않고 심사를 통과하는 상태 | S5 | 4 | 2 | 2.4 |
...
| **MK-2** | 유사 거래 **2년간 0건** — 평가 지연이 아니라 **평가 불가** | 산출 불가를 이유와 함께 받는 상태 | S3-a | 5 | 1 | **4.0** |
...
| ID | Pain | Needs | CJM | I | S | **AOS** 🔵 |
|---|---|---|:---:|:---:|:---:|---:|
| **BS-2** | QRM 심사에서 **출처 신뢰성이 인정 안 되면 인용 자체가 불가** | 법인 표준 인용 출처로 등록된 상태 | S4 | 5 | 2 | **3.0** |
| **BS-1a** | *(대형 딜)* 감정평가가 **법적 효력까지 제공** — 우리가 필요 없음 | — | S2 | 3 | 4 | 0.6 ⛔ |
| **BS-1b** | *(소형·중형 딜 — "훨씬 많음")* 공개 통계 + 사내 DB로 버팀 | 딜이 바뀌어도 같은 출처를 각주로 재사용 | S2 | 4 | 2 | 2.4 |
| **BS-3** | 고객사가 이의를 제기하면 그 딜 이후 중단 | 방법론 문서로 답할 수 있는 상태 | S7 | 3 | 3 | 1.2 |

### 🟢 조현우 — 회계법인 감사본부 11년차 *(신규 편입)*
...
**`인지 → 탐색 → 구매 → 사용 → 재구매`를 쓰지 않았습니다.** 끊기는 지점을 찾아 8구간으로 나눴고, 그중 셋은 일반 SaaS 여정에 없습니다. 🔵

| # | 단계 | 이 시장에서 왜 생겼나 |
|:---:|---|---|
| S1 | **결산·사건 압박 자각** | 문제를 상시 느끼지 않습니다. **사건이 있을 때만** 문제가 됩니다 |
| S2 | **대안 탐색** | *"감정평가를 더 발주할까 / 데이터를 살까"* — **경쟁사 비교가 아닙니다** |
| S3 | **파일럿 검증** ★ | 자기 자산을 넣어 **기존 감정평가액과 대조**. 이 시장은 시연이 아니라 대조로 판정합니다 |
...
**게이트키퍼가 조직마다 다릅니다** — 같은 자료를 돌려 쓸 수 없다는 뜻입니다.
...
> 🔴 **Emotional·Social Job이 실제 구매 동기입니다.** 이것이 *"감정평가법인이 파는 것은 값이 아니라 면책"* 이라는 경쟁력 검토 결론(§6.4)과 같은 축입니다. 발언 근거는 §1.2.
...
| **O9** | 감사 표본 추출 근거 | *"금액 순"* | **이상치 지목** | 표본 선정 사유 기재 | 조현우 |
...
| 조현우 | *"외부 전문가는 건당 비용이 커서 큰 건에만"* · **비용 주체 미확인** | **별도 계약 필요** *(§13-1)* |
...
> 🔴 **⑨는 겨루는 상대가 아니라 요건 제정자입니다.** 다른 축은 *"경쟁자를 이겨야"* 하는데, STO 축은 *"제도 요건을 정의하는 과정에 들어가야"* 합니다.

**그중 11사를 실명·수치로 확인했습니다.**

| 축 | 사업자 | 규모 | 핵심 비즈니스 모델 | 도출된 가치 선언 |
|---|---|---|---|---|
| **제도** | 한국부동산원 | 부동산 가격공시·국가통계 전담. R-ONE 포털 | **비영리 공적 기관** — 이용자 무료 | *"국가가 말하는 하나의 숫자를 무료로"* |
| | 대형 감정평가법인군 | 업계 **1조 1,629억**(2023) · 13개 법인 **9,218억**(2024)<br/>삼창 797억 · 경일 786억 · 미래새한 783억 | **건별 수수료** — 보수는 국토부 공고 기준 | *"값이 아니라 책임지는 사람을 판다"* |
| **국내 데이터** | **빅밸류** | 금융위 **지정대리인** · AI 시세를 **시중은행 담보심사에 공급** | B2B 데이터 구독 + **금융기관 업무 위탁** | ***"데이터로 판단의 기준을 만듭니다"*** *(공식)* |
| | 밸류맵·디스코 | 디스코 실거래 **3,000만 건** *(2차 출처)* | 프리미엄 구독 + 중개 연계 | *"정제 기술을 특허로 지킨다"* |
| **글로벌** | CoStar Group | **FY2025 매출 32억 달러(+19%)** · 부동산 기록 850만 건 | **구독이 매출의 95%+** | *"업무를 시작할 때 가장 먼저 켜는 화면"* |
| | Altus Group | 소프트웨어 **ARR 1.979억 달러(+10.6%)** · VMS ARR 1.669억 | 밸류에이션 SW 라이선스 + VMS 서비스 | *"계산하는 문법을 제공한다. 숫자는 당신이"* |
| **디지털** | **CF Benchmarks** | CME CF BRR이 **1,000억 달러+** 규제상품 기준 · 미국 현물 BTC ETF **11개 중 6개** · **FCA 등록** | 지수 라이선싱·산출대행 | *"규제기관이 인정한 기준가만이 기준가다"* |
| | Kaiko | 누적 투자 **7,700만 달러** · 기관 고객 200+ | 시장데이터·퀀트·지수 구독 | *"우리는 어느 거래소의 편도 아니다"* |
| | 크로스앵글(쟁글) | 시리즈B **170억 원** · 거래소 70+ · 발행사 3,000+ | 공시 플랫폼 + 데이터 구독 | *"법이 만들기 전에 우리가 공시를 만든다"* |
| **STO** | 증권사 진영 | 미래에셋컨설팅 → 코빗 **97.7%** 인수 · 미래에셋 홍콩 1,000억 디지털 채권 | 발행 주관·중개 수수료 + 리테일 앱 락인 | *"전통·디지털을 한 앱에서. 상품은 나중에"* |
| | 예탁결제원·협의체 | 전자증권법 **2026.1.15 통과 · 2.3 공포 · 2027.2.4 시행** | 제도 인프라 운영 | *"무엇이 객관적 가치평가인지를 정의한다"* |

**그리고 사라진 경쟁자가 있습니다 — 1세대 부동산 조각투자.** ✅

| 사업자 | 결과 | 사유 |
|---|---|---|
| **카사코리아** | 2026.8.10 종료 *(1개 매체 보도)* · 2025년 매출 **5,355만** · 순손실 **61억** | 혁신금융 지정 만료 후 **투자중개업 인가 미취득** |
| **펀블** | 플랫폼 종료·청산 | 동일 |
| **에이판다** | 에잇퍼센트 인수 | 재편 |

> 🔴 **제도화가 혁신 사업자를 걸러낸 역설입니다.** 함의 셋 — ① **규제 샌드박스는 시한부 유예** ② **인가가 필요 없는 데이터 공급자 포지션이 이 구조에서 살아남는 자리** ③ 조각투자 사업자 소멸로 **기초자산 평가·신탁 공백이 실제로 발생**했습니다.
...
| 4 | **재현성** | *Reproducible Valuation* | as-of 조회 + 방법론 버전 이력 + **과거 오차 이력 공개** | **K2** |
...
| 사후 검증 | 어렵다 | **as-of 재현 + 백테스트·오차 이력 공개** |
...
|---|---|
| **Customer Jobs**<br/>*(고객이 하려는 일)* | 분기 공시·NAV 산출 · 담보 재평가 · 시장감시 · 실사·입찰 · 딜 자문 보고서 작성 · **감사 검증** · 감독·투자심의 대응 |
| **Pains**<br/>*(고통)* | 가치 공백(KD-1) · 근거 부재(MK-1) · 재현 불가(JM-2) · 지연·장애(PS-2) · 보안 반려(KD-2) · QRM 미통과(BS-2) · 평가 불가(MK-2) · **독립 대조군 부재(CH-1)** |
| **Gains**<br/>*(기대 이득)* | 내 이름으로 방어하지 않아도 됨 · 소명 2개월→1일 · 안 재던 자산 커버 · *"우리만 이상한 게 아니다"* 를 숫자로 · **"데이터가 지목했다"고 조서에 기재** |
...
| **Gain Creators**<br/>*(이득 창출)* | 인용용 각주 포맷(F9) · 일괄 배치(F10) · 괴리 대시보드(F12) · **백테스트·오차 이력 공개(F15)** · 이상치+모집단 리포트(F17) |
| **Products & Services** | 평가 엔진 + **Evidence Layer** + **Confidence Layer** + **as-of 이력 저장소** + API/파일/온프레미스 전달 + **심사 통과 문서 패키지** |

> 💡 **Pain Relievers 9종 중 3종(F6·F7·F8)이 코드가 아니라 문서입니다.** 가치사슬 분석(§8.4)에서 *"난이도 최저인데 CJM 이탈 4건을 막는 활동"* 으로 확인됐습니다.
...
| 4 | **JTBD 인터뷰** | 7인 중 6인이 **책임 회피·외부 권위**를 말함 | 🟡 |
...
## 10.6 AOS × DOS 사분면 — 인터뷰 후 6종이 칸을 넘었습니다 🔵

`기준선: AOS 2.70 · DOS(SAM) 0.172`

| ID | AOS *(가설→실제)* | I−S | MR | DOS | 사분면 *(가설→실제)* |
|---|:---:|:---:|---:|---:|:---:|
...
| 사분면 | 실제 구성 | 처방 |
|---|---|---|
| 🔥 **Q1 즉시 착수** *(8종)* | KD-2 · KD-3 · PS-1 · PS-3 · JM-2 · MK-1 · MK-2 · MK-3 | **MVP 범위** |
| 💰 **Q2 크지만 덜 아픔** *(3종)* | PS-2 · JM-1 · JM-3 | 🔴 **"나중"이 아니라 "출시 첫날 있어야"** — 셋 다 경쟁 기준선 |
| 🛡️ **Q4 아프지만 작음** *(1종)* | BS-2 | 매출이 아니라 **쐐기·신뢰 방어** |
| ⚪ **Q3 후순위** *(6종)* | KD-1 · BS-1 · NG-1b · NG-2 · NG-3 · BS-3 | 후순위 · NG-1a는 **자원 회수** |

> 🔴 **MK 3종은 "Q1 승격"이 아니라 "Q1 경계에 걸침"으로 읽어야 합니다.** DOS 0.172는 중앙값에 정확히 걸친 경계값이라 기준선이 조금만 움직이면 다시 Q4로 내려갑니다. **순위가 아니라 "확인 필요"로 다룹니다.**
>
> 💡 **KD-1은 AOS 1.8 · Q3인데 MVP High(F10)입니다.** 모순이 아니라 KSF 때문입니다 — **일괄 배치 없이는 다른 기능이 동작하지 않습니다. 점수가 낮아도 인프라인 항목이 있습니다.**

## 10.7 방법론 자체의 한계 — 발견된 것 🔵
...
| **F15** 백테스트 + **과거 오차 이력** 공개 | 검증 기반 신뢰 · **인증 조달** | **5** ▲ | 3 | **✔** | **High** | ② 산출 운영 |
| **F16** 자산 마스터 매핑 온보딩 *(유상)* | 도입 마찰 제거 | 4 | 2 | **✔** | **Mid** | ⑤ 가치 유지 |
| **F17** 이상치 리포트 + **모집단 커버리지** | 대조 검증 — **독립 3인 요구** | **5** | 3 | ✖ | **Mid** | ② 산출 운영 |
...
| 층위 | 값 | 주의 |
|---|---|---|
| **TAM** *(자산 스톡)* | 리츠 AUM **127.5조** *(리츠 469개, 2026.6)* · 건설·부동산 기업대출 **361조** *(2024)* · 가상자산 시총 **87.2조** *(2025 말)* | 항목 간 중복 — **단순 합산(575.7조) 금지** |
...
| 세그먼트 | 페르소나 | 기관 수 | 평균 계약 | SAM 매출 | SAM MR | SOM MR | 도입 기간 | 진입 |
|---|---|---:|---:|---:|---:|---:|---|:---:|
| 거래소 | **박선우** | 18 | 1.5억 | **27억** | 0.256 | **0.295** | 2~4개월 | **1-b** |
| 은행·보험·저축은행 | **조민경** | 30 | 8,000만 | **24억** | 0.227 | 0.126 ▼ | 12~24개월 | 5 |
| 대형 AMC·리츠 | **김도현** | 25 | 6,000만 | **15억** | 0.142 | **0.202** | 6~18개월 | 4 |
| 증권사 부동산금융 | *(이재훈)* | 20 | 7,000만 | 14억 | — | — | — | — |
| 커스터디·수탁 | *(한지우)* | 9 | 9,000만 | 8억 | — | — | — | — |
| 부동산 플랫폼·STO | — | 40 | 2,000만 | 8억 | — | — | — | — |
| STO·RWA·증권사 | *(서태영)* | 15 | 4,000만 | 6억 | — | — | *제도 의존* | 병행 |
| AMC 내 특수자산 *(침투율 30%)* | **문경아** | — | — | 4.5억 | 0.043 | 0.061 | *(김도현 동반)* | 3 |
| 회계법인 딜 자문·감사 | **백서현·조현우** | 6 | 6,000만 | **3.6억** | 0.034 | 0.076 | 딜 단위 + QRM 6~9개월 | **1-a · 2** |
| **NPL 전업** ⚠️ | **남기훈** | 8 | **2,500만** ▼ | **2.0억** ▼ | **0.019** ▼ | 0.032 ▼ | 1~3개월 | **제외** |
| **합계** | | **171** | | **105.6억** | | | | |

> 🔴 **조민경만 SOM에서 비중이 41% 떨어집니다.** SAM 2위인데 도입이 12~24개월이라 3년 안에 13%밖에 못 잡습니다 — **로드맵을 SAM으로 짜면 3년을 기다리는 계획이 됩니다.**

**진입 순서 — DOS 순위가 아니라 전환 동인 순입니다**

| 순서 | 대상 | 근거 |
|:---:|---|---|
| **1-a** | **백서현** *(회계법인 딜 자문)* | 전환 ✅ · Habits ★★ · 유일 장벽 QRM · **각주가 곧 광고** |
...
| 플랜 | 월 단가 | 연 환산 | 대상 |
|---|---:|---:|---|
| API Basic | 300만 | 3,600만 | 단일 피드 |
| Institutional | 500~1,000만 | 6,000만~1.2억 | 기관 · 다수 종목 |
| **Enterprise / Exchange** | 1,000~2,000만+ | **1.2~2.4억+** | 거래소 · 시스템 결합 |
...
## 12.4 산정 방법론의 핵심 — "469개 리츠 ≠ 469개 고객"
...
| ID | 가설 | 검증 질문 | 방법 | 깨지면 |
|---|---|---|---|---|
| **H1** ✅ | 감사인이 우리 값을 근거로 받아들인다 | *"피감회사가 우리 데이터를 근거로 냈다면 받아들이겠는가?"* | **조현우 인터뷰 C6** 🟡 | **조건부 성립 — §13-1** |
...
**출처**: 조현우 응답지 🟡 *(AI 생성 가상 응답 — 실제 감사인 발언 아님)*
...
> 🔴 **C7이 조현우 편입 검토의 미해결 독립성 이슈에 답했습니다.**
...
>
> **F15(백테스트·오차 이력 공개)가 제품 기능이 아니라 인증 자산이었습니다.** K1 조달 수단에 QRM·보험·하이브리드에 이어 **네 번째 경로**가 추가됩니다.
...
**세 기둥은 모두 "고객이 원하는 것"에서 나왔습니다.** KSF 분석(§10.5)이 **고객이 말하지 않는 필수 요건 둘**을 지적합니다.
...
| 기업 부동산 자산관리팀 *(최도윤)* | 예산 항목도 반복성도 없음 — **간접 채널** |
| 일반 부동산 보유자 | 1회성·저빈도 — 세그먼트 맵이 명시적으로 제외 |
| NPL 전업 *(남기훈)* | **규제 트리거 없음** · WTP 연 2~3천 — 직접 영업 제외, 백서현 경유 간접 |
...
> 🚨 **정량 근거의 상당수가 AI 생성 가상 응답지 기반입니다**(🟡 표기). **인터뷰 7종 전부가 가상 응답이며 실제 고객 발언이 아닙니다.** 실제 인터뷰 후 이 문서를 갱신해야 하며, **현 상태로 투자·계약 근거로 쓸 수 없습니다.**
...
| 🟡 **가설** | **모든 인터뷰 인용문** · O1~O9 · WTP · 전환 판정 · 수용 임계값 | 🔴 **의사결정 근거 불가** |

## 17.2 개별 한계

| 한계 | 내용 |
|---|---|
| **인터뷰 데이터** | 🟡 **응답지 7종이 AI 생성 가상 응답.** H1~H8·H12가 그 검증 계획 |
...
> 📌 **이 문서는 사업성 조건부 성립 판정(§1.5) 위에 서 있습니다.** 네 조건(트리거 세그먼트 · 비용 구조 · 인증 조달 · 엔진 재사용) 중 하나라도 깨지면 이 VP도 함께 재검토해야 합니다.
