...
| 참조한 문서 | 이 문서에서 쓰인 곳 |
|---|---|
| [01-07 Porter 5 Forces](../01_porters-five-forces/07_분석_자산가치평가-인프라.md) · [01-08 사업성 판정](../01_porters-five-forces/08_검토_사업성-판정.md) · [01-09 경쟁사·가치선언](../01_porters-five-forces/09_리서치_경쟁사-가치선언.md) | §4 기존 대안 · §7 Proof |
...
> 💡 **세 번째 축이 이번에 추가됐습니다.** [경쟁사 리서치 §1](../01_porters-five-forces/09_리서치_경쟁사-가치선언.md)에서 확인한 대로, 토큰증권 협의체가 **"기초자산의 객관적 가치평가 가능성"** 을 제도 요건으로 명시하고 업계가 **"외부 가치평가 비용을 어떻게 낮출 것인가"** 를 과제로 올렸습니다. **다른 두 축과 달리 이 축에서는 수요가 제도로 강제됩니다.**
...
| **SAM** *(서비스 매출)* | **171개 기관 · 연 110억 원** *(NPL·회계법인 편입 반영 갱신값)* | 계약단가는 검증 전 추정치 |
| **SOM** *(3년)* | **40개 기관 · 연 25.4억 원 ARR** | SOM 목표 범위(20~50개·10~30억) 내 |
...
|---|---|---:|---:|---:|---|:---:|
| **박선우** | 거래소 (18개사) | 27억 | 0.245 | **0.276** | 2~4개월 | **1** |
| **백서현** | 회계법인 딜자문 (6곳) | 3.6억 | 0.033 | 0.071 | 딜 단위 | **1 (쐐기)** |
| **김도현** | 대형 AMC (25곳) | 15억 | 0.136 | **0.189** | 6~18개월 | 2 |
| **남기훈** | NPL 전업 (8곳) | 6.4억 | 0.058 | 0.094 | 1~3개월 | 2 |
| **조민경** | 은행·보험 (30곳) | 24억 | **0.218** | 0.126 ▼ | 12~24개월 | 3 |
| **문경아** | AMC 내 특수자산 | 4.5억 | 0.041 | 0.057 | *(김도현 동반)* | *(신뢰 방어)* |

> 🔴 **조민경만 SOM에서 비중이 42% 떨어집니다.** SAM 2위 세그먼트인데 도입이 12~24개월이라 3년 안에 13%밖에 못 잡습니다. **로드맵을 SAM으로 짜면 은행부터 가게 되는데 그것은 3년을 기다리는 계획입니다.**
...
graph LR
    ...
    N["<b>남기훈</b><br/>NPL 투자심사역<br/>평가값 = 매입 단가<br/><i>🔵 핵심·NPL</i>"]:::core
...
| ID | Pain | Needs | CJM | 가설→실제 AOS |
|---|---|---|:---:|:---:|
| **NG-2** | 실사 마감 안에 안 나오면 **품질과 무관하게 그 딜을 통째로 못 쓴다.** 2주에 240건 | 실사 데드라인 안에 담보물 전량 산정이 끝나는 상태 | S6 | 2.0 → **2.0** — |
| **NG-1** | **10년 쌓은 사내 DB가 있는데** 외부 데이터가 역량 외주화로 보인다 | 사내 모델과의 차이를 확인해 교차검증 도구로 쓰는 상태 | S2 | 0.6 → **1.6** ▲ |
| **NG-3** | 과거 낙찰 결과와 **소급 대조가 안 되면** 믿을 근거가 없다 | 과거 낙찰 대비 정확도를 도입 전에 확인하는 상태 | S3 | 2.4 → **1.8** ▼ |

> ⚠️ **세 지표(AOS 최하위·DOS 유일한 음수·사분면 Q4)가 모두 "자원 회수"라고 판정한 NG-1에서 반대 답이 나왔습니다** — *"대조군이면 쓸 수 있습니다."* 원인은 **우리가 NG-1을 전국 단일 값으로 본 것**이었습니다. 실제로는 **서울·경기 과잉충족(S 4~5) / 지방·비주거 미충족(S 2)** 으로, **하나의 Pain에 만족도가 두 개**인 경우를 AOS 수식이 표현하지 못했습니다.
...
| **남기훈** | ★★★☆☆ | ★★★★☆ | ★★★★☆ | ★★★★☆ | **7 < 8 ❌ 아니오(근소)** | **범위·가격을 좁히면 뒤집힘** — 지방·비주거 한정 + 차이 리포트 + 연 2~3천 |
...
[경쟁사 리서치](../01_porters-five-forces/09_리서치_경쟁사-가치선언.md)에서 검증한 결과입니다. **각 사의 가치 선언문은 그 문서에서 도출한 것**이며 빅밸류만 공식 슬로건입니다.
...
> 🔴 **[경쟁사 11사 가치 선언문을 모아 보니, 이 셋을 가치로 내세운 사업자가 하나도 없었습니다.](../01_porters-five-forces/09_리서치_경쟁사-가치선언.md)** 고객 조사(Pain 클러스터 1·2·4위)와 경쟁 지형(빈칸)이 같은 지점을 가리킵니다. **두 프레임이 독립적으로 같은 답을 내면 그 답의 신뢰도가 올라갑니다.**
...
graph LR
    ...
    N["<b>3. 남기훈</b> NPL<br/>차이 리포트 · 저가 한정<br/>백서현 각주로 우회 진입"]:::p3
    K["<b>4. 김도현</b> AMC<br/>전환 ❌ · KD-3 선결<br/>해자 형성 후 정면"]:::p4
    J["<b>5. 조민경</b> 은행<br/>참고용 → 규정 개정 2단계<br/>12~24개월"]:::p5
    ...
    classDef p3 fill:#FFCC99,stroke:#333;
...
| 순서 | 근거 |
|---|---|
| **1-a 백서현** | 전환 ✅ · Habits ★★ 최저 · 유일 장벽 QRM · **각주가 곧 광고**로 남기훈·투자사에 전파 |
| **1-b 박선우** | **SOM MR 0.276 최대** · 구매 2~4개월 · 결합 후 락인. 단 SLA·폴백 선결 |
| **2 문경아** | 전환 최강(9>5) · **신뢰 설계를 여기서 검증**하면 김도현 S7을 지킴 |
| **3 남기훈** | 직접 가면 S2(Build vs Buy)에서 막힘 → **백서현 각주로 우회** |
| **4 김도현** | **전환 ❌ — KD-3(감사 대응 근거) 없이는 불가.** 해자 형성 후 진입 |
| **5 조민경** | SAM 2위지만 SOM 비중 ▼42%. 참고용 선진입 → 규정 개정 2단계 |
...
| **⑤ 경쟁 지형** *(독립 확인)* | 11사 가치선언 중 재현성·정직성을 내세운 곳 **0곳** | [경쟁사 §7-4](../01_porters-five-forces/09_리서치_경쟁사-가치선언.md) |
...
| **NG-1** 사내 DB 과잉충족 | 세 지표 모두 "자원 회수" | *"대조군이면 쓸 수 있다"* | **하나의 Pain에 만족도가 두 개**(서울 과잉/지방 미충족)인 경우를 AOS가 표현 못 함 |
...
| **F17** 사내 모델 차이 리포트 (20%+ 괴리만) | NG-1 재해석 · O2 · **남기훈 제품 형태 자체** | **4** | 3 | **✖** | **Mid** |
...
| **F23** 권리관계·회수기간 레이어 | 남기훈 Q3 *"없으면 회수가 계산이 안 됩니다"* | 3 | **5** | **✖** | **Low** |
...
| **F23** 권리관계 레이어 | 등기·임차인 대항력 데이터 확보가 별도 사업. 남기훈 WTP(연 2~3천)로는 **투자 회수 불가** |
| **F17** 차이 리포트 | 남기훈 전환에 필수지만 그 세그먼트가 **저가·소수(8곳)**. 백서현 각주 우회가 먼저 |
...
| **남기훈 세그먼트 매출** | WTP 상한이 SAM 가정의 **1/3**로 확인 — SAM·DOS **하향 반영 필요** |
